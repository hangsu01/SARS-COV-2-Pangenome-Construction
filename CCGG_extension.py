...
class Nodes(dict):
    ...
    def getNode(self, nodename):
        ...
        if "hdr" in self.nodes[nodename].keys():
            ...
            self.nodes[nodename] = ujson.decode(hdr)
...
class Edges(dict):
    ...
    def getEdge(self, edgename):
        ...
        if "hdr" in self.edges[edgename].keys():
            ...
            self.edges[edgename] = ujson.decode(hdr)
...
# 
    ...
    def reconstructSequence(self, strain, path=0):
        ...
        # Iterate through the edges in order based on the destinations and outgoing edges from those destinations
        currentEdge = source
        firstNode = self.nextAnchor('SOURCE')
        firstPaths = self.getSubPath("SOURCE", firstNode)
        for firstitemlist, firststrains in firstPaths:
            if source in firstitemlist:
                firstpathseq = self.findsequence(firstitemlist, countinganchor= False)
                
                if strain.startswith('0'):
                    founder_on_that_Edge = list(set(edges[source]['strain']) & set('ABCDEFGHI'))[0]
                    numberN = int(nodes[firstNode][founder_on_that_Edge]) - 1 - len(firstpathseq)
                    conseq += "N" * numberN
                else:
                    founder_on_that_Edge = strain
                    numberN = int(nodes[firstNode][founder_on_that_Edge]) - 1 - len(firstpathseq)
                    conseq += "N" * numberN
        ...
        while True:
            ...
            if dst == "SINK":
                ...
                conseq += ("N" * int(edges[currentEdge].get("addNs",0)))
                return conseq.upper()
            else:
    ...
    def anchorPosList(self, ref = "MN908947"):   #populates global field: self.sortednodes but also returns it
        #will return a dictionary of anchors as keys with 8 [founder] values that show their coordinates
        tmp = []
        founder = []
        edgelist = self.outgoing['SOURCE']
        for edge in edgelist:
            founder += self.edges[edge]['strain']
        for i in sorted(self.nodes.keys(), key = lambda n: int(self.nodes[n][ref])):
            tup = [i]
            for f in founder:
                tup.append(int(self.nodes[i][f]))
            tmp.append(tuple(tup))
        dt = [("node", "U20")]
        for f in founder:
            dt.append((f, "i8"))
    ...
    def boundingAnchors(self, strain, startPos, endPos): #
        ...
        assert endPos >= startPos, 'endPos must be larger than or equal to the startpos'
    ...
    def Enumerate_anchor_by_anchor(self, start, end, sofar = [], strain = set('ABCDEFGHI')):
...
# Updated Aug 20, 2019, construct subsequences between each anchor pair then merge them together
    def getSubPath(self, startanchor, endanchor, seq = False, counting_anchor = True, init_strain = set('ABCDEFGHI')): 
        """Get all subpath between two anchor, return a list with tuple: if seq = "False" return (itemlist, shared strain) , else return (sequence, shared strain)
        ...
        Parameters:
            ...
            init_strain: <set> default founders
        """
        Subgraph = []
        anchor = startanchor
        ...
        if seq == True:
            ...
            for itemlist, strain in Subgraph:
                Sequence = self.findsequence(itemlist, countinganchor = counting_anchor)
...
## helper function for get floating node coordinate: start from floating nodes, traverse forward until the first anchor
    def Traverseforward(self, fnode, sofar = '', strain = set('ABCDEFGH')):
        ...
        if fnode.startswith('A') :   
            ...
            if len(strain)>0:
                self.subpath.append((sofar, strain))
            return 
...
# given node name, return dictionary for node coordinates: for floating node, return the coordinates for the base right before the floating node (1 base coordinates)
    def Nodecoordinates(self, node, strainlist = 'ABCDEFGHI'):
        """Given node name, return dictionary for linear coordinates for each node
        for floating node, by coordinates it refer to the base right before the floating node (1 base coordinates)
        Parameters:
    ...
    def entity_to_anchor(self, entity, offset, ref_node = 'proximal'):
        """Given Graph entity (anchor or edge), offset and strain attributes, return linear coordinates for the base position
        ...
        Strain_Offset = {}
        if entity.startswith('A'):
            if ref_node == "proximal": # default proximal anchor
                for s in 'ABCDEFGHI':
            ...
            node = self.edges[entity]['src']
            node_coord = self.Nodecoordinates(node)
            strainlist = set(self.edges[entity]['strain']) & set('ABCDEFGHI')
            if ref_node == "proximal": # default
                if node.startswith('A'):
                    for s in strainlist:
